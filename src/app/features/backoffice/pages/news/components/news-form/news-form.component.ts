...
export class NewsFormComponent implements OnInit, OnDestroy {
  ...
  getNews(id: number) {
    ...
    const getNewsSubscribe = this.httpService.get<NewResponse>(url).subscribe({
    ...
    this.subscriptions.push(getNewsSubscribe)
  }
  ...
  patch() {
    ...
    const partchSubscribe = this.httpPrivateService.patch<NewResponse>(url, body).subscribe({
    ...
    this.subscriptions.push(partchSubscribe)
  }
  ...
  post() {
    ...
    const postSubscription = this.httpPrivateService.post<NewResponse>(this.url, body).subscribe({
